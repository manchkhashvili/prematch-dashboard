...
> ordered `fault` -> `locked` -> `dom` -> `ev` -> `gap`, not by raw severity.
...
- **`dom`** — one price is **impossible beside another on the same screen**.
  `pickem_dominance`, `combo_dominance`, `vb_sets_dominance`. All three are
  containments: outcome set A sits inside B, and B is quoted longer. As
  certain as `locked` — raw prices, no devig, no model — but uncashable on
  its own, because it says which of two bets to take, not that either is
  worth taking. Ranked on **`dom_pct`**, the % by which the better bet is the
  longer price.
- **`gap`** — a contradiction with no money on it: a probability gap (pp) or
  a total that misses its children (pts). **No `ev_pct`.**

Order: `fault` -> `locked` -> `dom` -> `ev` -> `gap`.
...
The first cut put `pickem_dominance` in `gap` — reasoning that a free upgrade
is not a bet. Owner, on a `Pinheiros - Cascavel` row at 13.3 sitting under
every model-priced row on the board: *"they should be second before ev
betts."* Right, and the reasoning was wrong: **uncashable is not the same as
uncertain.**

> FT: the 0.0 handicap away @6.4 is LONGER than the 1X2 away @5.65 (ratio
> 1.133), but the 0.0 handicap voids the draw where the 1X2 loses to it - the
> better bet cannot be the longer price.

That is arithmetic on raw prices. No devig, no model, nothing to disagree
with. It belongs directly under the locks, and `combo_dominance` and
`vb_sets_dominance` are the same species — which also undid the previous
day's demotion of the HT/FT `combo_dominance` branch to `gap`. It was right
that it is not an EV; it was wrong that it is not certain.

`ml_vs_spread` stays in `gap`, and the distinction is the point: it says two
markets disagree **without saying which one is wrong**, so it names no bet.
A dominance row names the bet — it just does not promise the bet is good.

### Why the tier ranks on `dom_pct` and not severity

Two of the three dominance kinds gate on a model fair before firing (that
gate is what stops "the superset pays more" from firing on pairs where both
prices are simply bad), so **their severity is an EV**, while the unpriced
one's severity is a price improvement. Ranking the tier on severity would
rebuild the same category error one level down. Every dominance emitter
therefore hands over `dom_pct` — the size of the impossibility — and the
priced ones hand over `ev_pct` as well, so the EV column still shows the
money where money exists.
...
- **`combo_dominance`** — the totals branch gates on the atom model and
  ...
  true, and the bet can still be EV −40%. Both are `dom`; only the first
  carries an `ev_pct`.
...
Pinned by `tests/test_flag_rank.py` (60 tests), including a totality guard:
