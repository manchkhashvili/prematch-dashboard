...
class _FakeGame:
    def __init__(self, i, *, loadinfo="v1", market_count=300):
        from datetime import datetime, timedelta, timezone
        self.event_id = f"E{i}"
        self.home, self.away = f"H{i}", f"A{i}"
        ...
        # In-band by default so these tests exercise the budget/cache
        # behaviour rather than the market-count band — that has its own file,
        # tests/test_ladder_market_band.py. Must stay under the shipped ceiling.
